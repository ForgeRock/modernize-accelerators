...
public class LegacySMLoginTest {
	...
	@Mock
	LegacySMLogin.LegacyFRConfig smConfig;
	...
	private final JsonValue secureState = JsonValue.json("");

	@BeforeMethod
	public void before() throws SMSException, SSOException {
		initMocks(this);
		when(secrets.getRealmSecrets(realm)).thenReturn(null);
		when(agentAPI.init(initDef)).thenReturn(AgentAPI.SUCCESS);
	...
	public void shouldReturnTrueAgentApiInitFailure() throws NodeProcessException {
		LegacySMLogin node = new LegacySMLogin(smConfig, nodeId, realm, secrets, serviceRegistry);
		try {
	...
	public void testInvalidConfigurationAgent4x() throws NodeProcessException, SMSException, SSOException {
		...
		try {
			...
			assertEquals("LegacySMLogin::process > Configuration is not valid for the selected agent type",
					e.getMessage());
		}
