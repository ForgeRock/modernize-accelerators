...
public class LegacySMValidateTokenTest {
	...
	@Mock
	LegacySMValidateToken.LegacyFRConfig config;
	...
	private LegacySMValidateToken node;
	...
	public void setup() throws Exception {
		...
		node = new LegacySMValidateToken(config, realm, secrets);
	}
	...
	public void testInvalidConfigurationAgent4x() throws NodeProcessException {
		...
		try {
			...
			assertThat(e).isInstanceOf(NodeProcessException.class).hasMessage(
	...
	@Test
	public void testProcessFailedNoCookieFound() throws NodeProcessException {
		given(config.is4xAgent()).willReturn(true);
		...
		given(context.request.cookies.get(config.legacyCookieName())).willReturn(null);
	...
	@Test
	public void testProcessFailedCookieFoundAgentInitFailed() throws NodeProcessException {
		given(config.is4xAgent()).willReturn(true);
		given(config.policyServerIP()).willReturn("127.0.0.1");
		given(config.accountingPort()).willReturn(ACCOUNTING_PORT);
		given(config.authenticationPort()).willReturn(AUTHENTICATION_PORT);
		...
		given(context.request.cookies.get(config.legacyCookieName())).willReturn("cookievalueexpected");

		try {
			node.process(getContext(sharedState));
		} catch (Exception e) {
			assertThat(e).isInstanceOf(NodeProcessException.class)
					.hasMessage("SM AgentAPI init failed with status: -1");
		}
	}
