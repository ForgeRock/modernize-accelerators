...
| AM       | Node                 | Legacy-SM-Create FR User                                |   Updates the states (Shared state, Transient state - if necessary) with the provided attributes |
...
| AM       | Authentication Tree  | siteminderMigration                                     | Implements the migration login and bi-directional SSO                                                                                                                                        |
...
| Minimum connections       | 1                    | Number of initial connections for Siteminder SDK.                                                                                                                  |
...
#### 3.4.2. Legacy-SM-Create FR User
Custom node provided in the SSO toolkit. Executes the OAM11G get user information call and adds the user’s details on OBJECT_ATTRIBUTES on shared state and transient state. The nodes uses the following shared state attributes: 
...
#### 3.4.3. Legacy-SM-Login
Custom node provided in the SSO toolkit. Validates the credentials (username and password) entered by the user against the legacy IAM system via an SDK/API call. The default node uses a POST API call with the username and password fetched from the Shared and Transient states. The URL is configurable. The node expects a successful response of 200 OK and a specific cookie to be present in the response; the cookie name is configurable. The node is vendor-specific and is flexible enough to be tailored for each vendor.
...
#### 3.4.4. Add Attributes To Object Attributes
